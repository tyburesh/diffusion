...
class Diffusion:
	...
	def initialize_kernel(self):
		...
		self.init_curand = self.mod.get_function('init_curand')
		self.generator = curandom.XORWOWRandomNumberGenerator()
		self.init_curand(self.generator.state, self.randoms, block = (self.block_dims, self.block_dims), grid = (self.grid_dims, self.grid_dims))
		print('Self.randoms = ', self.randoms.get())
		#self.states = drv.mem_alloc(size*pycuda.characterize.sizeof('curandStateXORWOW', '#include <curand_kernel.h>'))
		#print('Size of states: ', sys.getsizeof(self.states))
